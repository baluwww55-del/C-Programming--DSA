...
// dequeue algorithm :
dequeue ( queue[], front,rear) // we have taken one queue , first element and last element to perform dequeue operation .
{
    if(front == rear == -1)     // this shows the array is an empty underflow condition occurs.
    printf("underflow");
    return ;

    if (front == rear) // this shows the array and front = same element and deletion in an same element . 
    front = rear = -1;
    else 
    front = (front+1)modn;
}
...
// run time complexity of the algorithms is known as O(1) :
// why because only the several steps has been taken to delete the elemement in an queue data structure :